...
path_to_last_room_with_unexploreds.push(['s'])
player.travel('n')
last_move = 'n'
last_room = 0
count = 0

while len(visited) < len(roomGraph) and count < 30:

  count += 1
  r_id = player.currentRoom.id
  print(visited)
  print(last_room)
  print(f"current room id is {r_id}")
  exits = player.currentRoom.getExits()
  current_path = path_to_last_room_with_unexploreds.top()
  print(f"path_to_unexplored is {current_path}")
  ...
  if not r_id in visited:
    ...
    if len(exits) == 1 and exits[0] == opposites[last_move]:
      print("Got to a dead end.")
      # traversalPath.append(opposites[last_move])
      # player.travel(opposites[last_move])
      # last_move = opposites[last_move]
      # last_room = r_id
  ...
  # If we've been to this room before, make whichever decisions
    ...
    print("We've been to this room before")
    unexplored_count = 0
    found_first_unexplored = False
    # print(visited[r_id])
    for e in visited[r_id]:
      if visited[r_id][e] == '?':
        print("direction hasn't been explored")
        unexplored_count += 1
        if not found_first_unexplored:
          found_first_unexplored = True
          traversalPath.append(e)
          ...
          last_move = e
          last_room = r_id
          # If there are previous roomswith unexplored paths
          if path_to_last_room_with_unexploreds.size() > 1:
            old_path = path_to_last_room_with_unexploreds.pop()
            old_path.append(last_move)
            path_to_last_room_with_unexploreds.push(old_path)
